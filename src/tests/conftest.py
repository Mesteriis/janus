...
if "docker" not in sys.modules:
    ...
    class _DummyContainers:
        ...
        def run(self, image, cmd, name=None, detach=True, restart_policy=None, network_mode=None, volumes=None, environment=None):
...
def client_factory(monkeypatch, tmp_path):
    def _make(**env_overrides):
        ...
        monkeypatch.setattr(provisioning, "_run_caddy_validate", _noop_validate)
        monkeypatch.setattr(provisioning, "ensure_tunnel_running", lambda: {"status": "running"})
        monkeypatch.setattr(provisioning, "sync_cloudflare_from_routes", _noop_sync)
